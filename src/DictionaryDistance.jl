...
# function precition
function nonzerovec(vec::SparseVector{<:Number})
...
function _plot_nice_side_by_side_view_of_sparse_results(
        X, X_res; filter_smallest_quantile = false)
    if filter_smallest_quantile
        X[X .< 0.25] .= 0
        X_res[X_res .< 0.25] .= 0
    end
    col_idx = 1:20
    row_idx = findall((sum.(!iszero, eachrow(X[:, col_idx])) .!= 0) .||
                      (sum.(!iszero, eachrow(X_res[:, col_idx])) .!= 0))
    S = (length(row_idx), 1)
